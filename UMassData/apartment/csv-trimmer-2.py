...
for i in range(1, 115):
    if (i != 3 and i!= 6 and i != 21 and i != 65 and i != 94 and i != 112):
        with open("Apt" + str(i) + "_trimmed.csv", "r") as file1:
            reader = csv.reader(file1, delimiter=',', quotechar='|')
            stop = 0
            date = least
            for row in reader:
                if stop == 0 and float(row[1]) == 0:
...
for i in range(1, 115):
    if (i != 3 and i != 6 and i != 21 and i != 65 and i != 94 and i != 112):
        with open("Apt" + str(i) +"_trimmed.csv", "r") as file1:
            reader = csv.reader(file1, delimiter=',', quotechar='|')
            with open("Apt" + str(i) + "_trimmed-2.csv", "w", newline='') as newfile:
                writer = csv.writer(newfile)
                for row in reader:
                    if datetime.datetime.strptime(row[0], "%Y-%m-%d %H:%M:%S") > least:
                        writer.writerow(row)
                    else:
                        print("Fail: ", row[0])
"""
